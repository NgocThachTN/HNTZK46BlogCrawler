...
async function runCrawler() {
  ...
  try {
    ...
    const generateYearlySvg = (year: number, blogsList: BlogPost[], imagesDir: string, authorId?: string, memberSlug?: string): { svgPath: string; totalBlogs: number } => {
    ...
    // Determine the newest year in database
    const yearsSet = new Set<number>();
    sortedBlogs.forEach(blog => {
      try {
        const year = Number(blog.date.split(' ')[0].split('.')[0]);
    ...
    const availableYears = Array.from(yearsSet).sort((a, b) => b - a);
    const newestYear = availableYears[0] || 2026;

    let memberGridsMarkdown = `### Member Contribution Heatmaps (${newestYear})\n\n`;
    memberGridsMarkdown += `Click on any member below to view their detailed blog contribution heatmap for the year ${newestYear}:\n\n`;
    ...
    members.forEach((member, index) => {
      ...
      memberSortedYears.forEach(year => {
        generateYearlySvg(year, sortedBlogs, CONTRIBUTIONS_DIR, member.id, slug);
      });
      
      const memberNewestYear = memberSortedYears[0] || newestYear;
